...
struct ITime{
    ...
    function ITime(counter, period, epoch)
        if counter isa Rational && counter.den == 1
            return new{Int, typeof(period), typeof(epoch)}(
                counter.num,
                period,
                epoch,
            )
...
function Base.://(t::T1, num::IntegerOrRatio) where {T1 <: ITime}
    ...
    ITime(t.counter // num, t.period, t.epoch)
end
...
Base.oneunit(t::T) where {T <: ITime} = ITime(1, t.period, t.epoch)
Base.zero(t::T) where {T <: ITime} = ITime(0, t.period, t.epoch)
