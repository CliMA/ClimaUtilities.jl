...
```
julia> _date_in_range(DateTime(1993, 19, 11), DateTime(1993, 1, 11), DateTime(1993, 12, 11))
true
...
"""
    _interpolable_range(dates_period_left, dates_period_left)
...
julia> _interpolable_range(dates_period_left, dates_period_left, period)
...
function TimeVaryingInputs.evaluate!(
    ...
    if in_interpolable_region
        # Note, we use PeriodicCalendar to repeat the period, so that we effectively
        # evaluate the given time on a different period
        ...
        TimeVaryingInputs.evaluate!(
            ...
            itp,
            time,
            LinearInterpolation(PeriodicCalendar(period, period_left)),
        )
        dest .*= (1 - coeff)
        TimeVaryingInputs.evaluate!(
            tmp_field1,
            itp,
            time,
            LinearInterpolation(PeriodicCalendar(period, period_right)),
        )
        tmp_field1 .*= coeff
        dest .+= tmp_field1
        return nothing
