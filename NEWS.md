...
### Added support for composing input variables
...
### New function `OutputPathGenerator.most_recent_counter`

`OutputPathGenerator.most_recent_counter` is a small utility that can be used to
detect the last output folder that was generated by `ActiveLinkStyle` in a given
base folder. It takes `name_rx` as one of the arguments. The default scheme
currently used by `ActiveLinkStyle` can be detected with:
```julia
