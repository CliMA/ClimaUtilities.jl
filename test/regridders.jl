...
@testset "InterpolationsRegridder incorrect dimensions" begin
    ...
    reg_horz_reversed = Regridders.InterpolationsRegridder(
        horzspace;
        dim_transforms = (identity, reverse),
    )
    ...
    dim_transforms = (identity, reverse, reverse)
    ...
    # test that error is thrown when incorrect number of dimensions are passed
    reg_hv = Regridders.InterpolationsRegridder(
        hv_center_space;
        extrapolation_bc,
        dim_transforms = (identity, identity, identity, identity),
    )
    @test_throws "Only 2D and 3D data is supported" Regridders.regrid(
        reg_hv,
        data_z3D,
        (lon, lat, z, z),
    )
